...
(L.Map as any).SmoothWheelZoom = L.Handler.extend({
    ...
    _onWheeling: function (e: WheelEvent) {
        ...
        if (this._goalZoom > map.getMaxZoom()) {
    ...
    _onWheelEnd: function () {
        ...
        // Set the final zoom level without snapping to integer
        var map = this._map;
        if (Math.abs(this._goalZoom - map.getZoom()) > 0.001) {
            // Force the zoom to the exact goal zoom without validation
            map._zoom = this._goalZoom;
            map._move(map.getCenter(), this._goalZoom);
        }
    ...
    _updateWheelZoom: function () {
        ...
        // Don't round the zoom level to maintain smooth decimal values
